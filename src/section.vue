...
export default {
  ...
  computed: {
    ...
    customStyle() {
      const shadowColorAfter = this.section.data.animAtHover
        ? this.section.data.shadowColorAfter
        : this.section.data.shadowColor;
      const pixelsToScrollTop = `-${
        this.section.data.animAtHover
          ? this.section.data.pixelsToScrollTop
          : "0"
      }px`;
      const contentHeight = this.section.data.isCardFitContent
        ? "auto"
        : "100%";
      return {
  ...
  methods: {
    ...
    async openOptions() {
      ...
      if (result.shadowColorAfter) {
        ...
        this.section.data.shadowColorAfter = this.computeShadow(
          result.shadowColorAfter
        );
        updateSection = true;
      }
